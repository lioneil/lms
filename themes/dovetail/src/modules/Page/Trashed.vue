<template>
  <admin>
    <metatag :title="trans('Trashed Page')"></metatag>
  ...
  data: () => ({
    ...
    resources: {
      ...
      headers: [
        ...
        { text: trans('Category'), align: 'left', value: 'category', class: 'text-no-wrap muted--text' },
  ...
  methods: {
    ...
    bulkRestoreResources () {
      ...
      axios.patch(
        ...
        this.hideDialog()
        this.showSnackbar({
          text: trans_choice('Page successfully restored', this.tabletoolbar.bulkCount)
        })
      }).catch(err => {
        this.errorDialog({
          width: 400,
    ...
    restoreResource (item) {
      ...
      axios.patch(
        ...
        this.getPaginatedData(null)
        this.showSnackbar({
          text: trans_choice('Page successfully restored', 1)
        })
      })
    },
    ...
    deleteResource (item) {
      ...
      axios.delete(
        ...
        this.hideDialog()
        this.showSnackbar({
          text: trans_choice('Page successfully deleted', 1)
        })
      }).catch(err => {
        this.errorDialog({
          width: 400,
