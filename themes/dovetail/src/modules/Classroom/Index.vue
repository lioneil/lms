<template>
  <admin>
    <metatag :title="trans('All Classrooms')"></metatag>

    <page-header>
      <template v-slot:utilities>
        <can code="classrooms.trashed">
          <router-link
            :to="{name: 'classrooms.trashed', query: {from: $route.fullPath}}"
            class="dt-link t-d-none mr-4"
            exact
            tag="a"
            >
            <v-icon small left>mdi-delete-outline</v-icon>
            <span v-text="trans('Trashed Classrooms')"></span>
          </router-link>
        </can>
      </template>

      <template v-slot:action>
        <can code="classrooms.create">
          <div v-show="resourcesIsNotEmpty">
            <v-btn
              ...
              large
              >
              <v-icon small left>mdi-google-classroom</v-icon>
    ...
    <v-card>
      ...
      <v-slide-y-reverse-transition mode="out-in">
        <template v-if="resourcesIsNotEmpty">
          <v-data-table
            ...
            <template v-slot:item.name="{ item }">
              <router-link class="t-d-none" tag="a" exact :to="{ name: 'classrooms.edit', params: {id: item.id} }">
            ...
            <template v-slot:item.action="{ item }">
              <action-buttons
                @item:destroy="askUserToDestroyClassroom"
                name="classrooms"
                v-model="item"
                :details="false"
              ></action-buttons>
        ...
        <template v-else>
          <empty-classroom-list></empty-classroom-list>
        </template>
...
export default {
  components: {
    EmptyClassroomList: () => import('./partials/EmptyClassroomList'),
  },
  ...
  data: () => ({
    ...
    resources: {
      ...
      headers: [
        ...
        { text: trans('# of Courses attached'), align: 'center', value: 'meta.courses:total', class: 'text-no-wrap' },
  ...
  methods: {
    ...
    bulkTrashResource () {
      ...
      axios.delete($api.destroy(null), { data: { id: selected } })
        .then(response => {
          ...
          this.hideDialog()
          this.showSnackbar({
            text: trans_choice('Classroom successfully moved to trash', this.tabletoolbar.bulkCount)
          })
        })
        .catch(err => {
          this.errorDialog({
    ...
    askUserToDestroyClassroom (item) {
      this.showDialog({
        color: 'warning',
    ...
    destroyResource (item) {
      ...
      axios.delete($api.destroy(item.id))
        .then(response => {
          ...
          this.showSnackbar({
            text: trans_choice('Classroom successfully moved to trash', 1)
          })
          this.hideDialog()
        })
        .catch(err => {
