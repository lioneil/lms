<template>
  <admin>
    <metatag :title="trans('Trashed Discussion')"></metatag>
        ...
        <template v-else>
          <empty-state>
            <template v-slot:actions>
              <v-btn
                :to="{name: 'threads.index'}"
                class="mb-10"
                color="primary"
                exact
                large
                >
                {{ trans('Go back to all discussions') }}
              </v-btn>
            </template>
          </empty-state>
        </template>
  ...
  methods: {
    ...
    bulkRestoreResources () {
      ...
      axios.patch(
        ...
        this.hideDialog()
        this.showSnackbar({
          text: trans_choice('Discussion successfully restored', this.tabletoolbar.bulkCount)
        })
      }).catch(err => {
        this.errorDialog({
          width: 400,
    ...
    restoreResource (item) {
      ...
      axios.patch(
        ...
        this.getPaginatedData(null)
        this.showSnackbar({
          text: trans_choice('Discussion successfully restored', 1)
        })
      })
    },
    ...
    deleteResource (item) {
      ...
      axios.delete(
        ...
        this.hideDialog()
        this.showSnackbar({
          text: trans_choice('Discussion successfully deleted', 1)
        })
      }).catch(err => {
        this.errorDialog({
          width: 400,
