    ...
    <v-card color="transparent" flat>
      <v-card-text class="px-0">
        <div class="d-flex justify-space-between">
          <div>
            <h4 class="mb-3" v-text="trans('Material File')"></h4>
          </div>
            ...
            <div class="text-right mb-3">
              <context-prompt class="d-inline-block">
                ...
                <v-card max-width="280">
                  <v-card-title v-text="trans('Remove File') "></v-card-title>
                  <v-card-text v-text="trans('The File will be permanently removed from storage. Are you sure you want to proceed?')"></v-card-text>
                  <v-card-actions>
                    <v-spacer></v-spacer>
      ...
      <template v-if="hasOldValue">
        <v-card-text class="pa-0">
          <input type="hidden" name="uri" :value="dataset">
      ...
      <template v-else>
        <v-card-text class="pa-0">
          <validation-provider
            :name="trans('uri')"
            rules="required|ext:PDF,TXT,PPT,PPTX,DOC,DOCX"
            v-slot="{ errors }"
            vid="uri"
            >
            <v-file-input
