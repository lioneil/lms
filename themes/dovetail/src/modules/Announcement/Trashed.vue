<template>
  <admin>
    <metatag :title="trans('Trashed Announcement')"></metatag>
        ...
        <template v-if="resourcesIsNotEmpty">
          <v-data-table
            ...
            <template v-slot:item.title="{ item }">
              <span class="muted--text" v-text="trans(item.title)"></span>
            </template>
            <!-- Title -->

            <!-- Category -->
            <template v-slot:item.category="{ item }">
              <category class="muted--text" v-model="item" :categories="resources.categories"></category>
            </template>
            <!-- Category -->
          ...
          <empty-state>
            <template v-slot:actions>
              <v-btn
                ...
                exact
                large
                v-text="trans('Go back to all announcements')"
                >
              </v-btn>
            </template>
          </empty-state>
...
export default {
  ...
  data: () => ({
    ...
    resources: {
      ...
      selected: [],
      headers: [
        { text: trans('Title'), align: 'left', value: 'title', class: 'text-no-wrap muted--text' },
        { text: trans('Category'), align: 'center', value: 'category', class: 'text-no-wrap muted--text' },
  ...
  methods: {
    ...
    bulkRestoreResources () {
      ...
      axios.patch(
        ...
        this.hideDialog()
        this.showSnackbar({
          text: trans_choice('Announcement successfully restored', this.tabletoolbar.bulkCount)
        })
      }).catch(err => {
        this.errorDialog({
          width: 400,
    ...
    restoreResource (item) {
      ...
      axios.patch(
        ...
        this.getPaginatedData(null)
        this.showSnackbar({
          text: trans_choice('Announcement successfully restored', 1)
        })
      })
    },
    ...
    deleteResource (item) {
      ...
      axios.delete(
        ...
        this.hideDialog()
        this.showSnackbar({
          text: trans_choice('Announcement successfully deleted', 1)
        })
      }).catch(err => {
        this.errorDialog({
          width: 400,
