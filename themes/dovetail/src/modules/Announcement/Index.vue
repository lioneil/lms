<template>
  <admin>
    <metatag :title="trans('All Announcements')"></metatag>

    <page-header>
      <template v-slot:utilities>
        <can code="announcements.trashed">
          <router-link
            :to="{name: 'announcements.trashed', query: {from: $route.fullPath}}"
            class="dt-link t-d-none mr-4"
            exact
            tag="a"
            >
            <v-icon small left>mdi-delete-outline</v-icon>
            <span v-text="trans('Trashed Announcements')"></span>
          </router-link>
        </can>
      </template>

      <template v-slot:action>
        <can code="announcements.create">
    ...
    <v-card>
      ...
      <v-slide-y-reverse-transition mode="out-in">
        <template v-if="resourcesIsNotEmpty">
          <v-data-table
            ...
            <template v-slot:item.title="{ item }">
              <router-link class="t-d-none" tag="a" exact :to="{ name: 'announcements.edit', params: {id: item.id}, query: {from: $route.fullPath} }">
            ...
            <template v-slot:item.action="{ item }">
              <action-buttons
                name="announcements"
                @item:destroy="askUserToDestroyAnnouncement"
                v-model="item"
              ></action-buttons>
            </template>
          </v-data-table>
        ...
        <template v-else>
          <empty-announcement-list></empty-announcement-list>
        </template>
...
export default {
  components: {
    ...
    EmptyAnnouncementList: () => import('./partials/EmptyAnnouncementList'),
  },
  ...
  methods: {
    ...
    bulkTrashResource () {
      ...
      axios.delete($api.destroy(null), { data: { id: selected } })
        .then(response => {
          ...
          this.hideDialog()
          this.showSnackbar({
            text: trans_choice('Announcement successfully moved to trash', this.tabletoolbar.bulkCount)
          })
        })
        .catch(err => {
          this.errorDialog({
    ...
    askUserToDestroyAnnouncement (item) {
      this.showDialog({
        color: 'warning',
        ...
        text: ['Some data related to announcement will still remain.', trans('Are you sure you want to move :name to Trash?', {name: item.title})],
        buttons: {
