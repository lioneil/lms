  ...
  <admin>
    ...
    <template v-slot:appbar>
      <appbar-action-buttons
        ...
        router-name="announcements.categories.index"
        v-model="resource"
    ...
    <page-header :back="{ to: { name: 'announcements.categories.index' }, text: trans('Categories') }">
...
<script>
import $api from '@/modules/Announcement/routes/api'
  ...
  methods: {
    ...
    submit (e) {
      ...
      axios.post(
        ...
        this.showSnackbar({
          text: trans('Announcement Category updated successfully'),
        })

        this.showSuccessbox({
          text: trans(`Updated Announcement Category ${this.resource.data.name}`),
          buttons: {
            show: {
              code: 'announcements.categories.index',
              to: { name: 'announcements.categories.index', params: { id: this.resource.data.id } },
              icon: 'mdi-tag-outline',
              text: trans('View All Announcement Categories'),
            },
            create: {
              code: 'announcements.categories.create',
              to: { name: 'announcements.categories.create' },
              icon: 'mdi-tag-plus-outline',
              text: trans('Add New Announcement Category'),
            },
          },
        })
