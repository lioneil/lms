<template>
  <admin>
    <metatag :title="trans('Trashed Assessments')"></metatag>
          ...
          <empty-state>
            <template v-slot:actions>
              <v-btn
                ...
                large
                >
                {{ trans('Go back to all assessments') }}
              </v-btn>
            </template>
          </empty-state>
        </template>
  ...
  methods: {
    ...
    bulkRestoreResources () {
      ...
      axios.patch(
        ...
        this.hideDialog()
        this.showSnackbar({
          text: trans_choice('Assessment successfully restored', this.tabletoolbar.bulkCount)
        })
      }).catch(err => {
        this.errorDialog({
          width: 400,
    ...
    restoreResource (item) {
      ...
      axios.patch(
        ...
        this.getPaginatedData(null)
        this.showSnackbar({
          text: trans_choice('Assessment successfully restored', 1)
        })
      })
    },
    ...
    deleteResource (item) {
      ...
      axios.delete(
        ...
        this.hideDialog()
        this.showSnackbar({
          text: trans_choice('Assessment successfully deleted', 1)
        })
      }).catch(err => {
        this.errorDialog({
          width: 400,
