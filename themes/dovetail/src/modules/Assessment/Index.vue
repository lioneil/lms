<template>
  <admin>
    <metatag :title="trans('All Assessments')"></metatag>

    <page-header>
      <template v-slot:utilities>
        <can code="assessments.trashed">
          <router-link
            :to="{name: 'assessments.trashed', query: {from: $route.fullPath}}"
            class="dt-link t-d-none mr-4"
            exact
            tag="a"
            >
            <v-icon small left>mdi-delete-outline</v-icon>
            <span v-text="trans('Trashed Assessments')"></span>
          </router-link>
        </can>
      </template>

      <template v-slot:action>
        <can code="assessments.create">
          <div v-show="resourcesIsNotEmpty">
            <v-btn
              ...
              large
              >
              <v-icon small left>mdi-ballot-outline</v-icon>
        ...
        <template v-if="resourcesIsNotEmpty">
          <v-data-table
            ...
            <template v-slot:item.title="{ item }">
              <router-link class="t-d-none" tag="a" exact :to="{ name: 'assessments.edit', params: {id: item.id} }">
            ...
            <template v-slot:item.action="{ item }">
              <action-buttons
                @item:destroy="askUserToDestroyAssessment"
                name="assessments"
                v-model="item"
                :details="false"
              ></action-buttons>
...
export default {
  ...
  methods: {
    ...
    bulkTrashResource () {
      ...
      axios.delete($api.destroy(null), { data: { id: selected } })
        .then(response => {
          ...
          this.hideDialog()
          this.showSnackbar({
            text: trans_choice('Assessment successfully moved to trash', this.tabletoolbar.bulkCount)
          })
        })
        .catch(err => {
          this.errorDialog({
    ...
    askUserToDestroyAssessment (item) {
      this.showDialog({
        color: 'warning',
    ...
    destroyResource (item) {
      ...
      axios.delete($api.destroy(item.id))
        .then(response => {
          ...
          this.showSnackbar({
            text: trans_choice('Assessment successfully moved to trash', 1)
          })
          this.hideDialog()
        })
        .catch(err => {
